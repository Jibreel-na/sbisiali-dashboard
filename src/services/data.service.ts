...
export class DataService extends ApiService {
    ...
    getRequests(model: PaginationModel) {
        return this.restRequest(null, `${this.baseUrl}/v1/request?&status=&limit=${model.limit}&page=${model.page}&sortBy=status:desc&requestTo=&requestby=&duration&type_hour=&post_story=&type=1`, null, 'GET');
    }

    getAdvertisings(model: PaginationModel) {
        return this.restRequest(null, `${this.baseUrl}/v1/advertising`, null, 'GET');
    }
    ...
    updateAdvertising(model: AdModel, type: string = 'PUT') {
        return this.restRequest(model, `${this.baseUrl}/v1/advertising/${model._id}`, null, type);
    }

    updateRequest(model: RequestModel, type: string = 'PUT') {
        return this.restRequest(model, `${this.baseUrl}/v1/request/${model._id}`, null, type);
    }
